...
- Set `TESTCONTAINERS_RYUK_DISABLED` ONLY behind `if (process.env.CI === 'true')` — never unconditionally. Locally, Ryuk must stay enabled so containers reap themselves.
