...
## Step 2c: Model Policy (fixed — no selection)
<!-- Stage 1: Research, Todo 2 -->

There is no model-profile question to ask. **Fable access is blocked**, so model assignment is fixed
for every run and depth is differentiated by *effort*, not by model:

- deep judgment roles (spec/code/product reviewers + `deep-analyst`) → **Opus**, effort `max`
- standard reviewers + doc-writers → **Opus**, effort `high`
- implementers → **Sonnet** (`deep`/`standard`/`fast` = effort `max`/`high`/`low`)

Nothing to ask or persist here — proceed to the next step.
