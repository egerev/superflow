...
12. **Heartbeat check every orchestrator turn.** Before any tool call that advances work, read `.superflow-state.json.heartbeat.must_reread`. If any listed file is NOT in current context, Read it first. If `heartbeat.updated_at` is >30 min old relative to now, emit a fresh heartbeat snapshot. Heartbeat is defense against compaction drift — skipping it defeats Rule 5 (re-read phase docs at sprint boundaries).
