...
3. **Unified Review before every PR** (2 agents for standard/critical sprints; single Technical reviewer for light-mode sprints): (When `context.use_workflows=true`, steps 1-3 may run via the saved `/superflow-review` workflow — see `references/workflow-orchestration.md`; steps 4-9 unchanged.)
   ...
   7. Write `.par-evidence.json`: `{"sprint":N,"claude_product":"ACCEPTED","technical_review":"APPROVE","docs_update":"UPDATED|UNCHANGED","docs_review":"PASS","provider":"codex|code-review-skill|split-focus|workflow-review","ts":"..."}`
...
14. **Release Gate is mandatory before Phase 3 when the project is runnable (web/backend-only).** Library projects substitute the P2 coverage threshold (verdict=SKIPPED, gate passes). The gate runs ONCE after the sprint loop + holistic review (if required), BEFORE the Completion Report. It writes `.superflow/release-gate/verdict.json`; Phase 3 refuses merge unless `verdict=PASS` (or `SKIPPED` for library). **No vacuous pass**: a web project with charter journeys but zero executed specs → verdict=FAIL. Per-journey coverage is checked by stable `spec_tag` ID, never by total count. Run `bash tools/release-gate.sh` (pure-computation helper, bash+jq only) with pre-assembled `--journeys` and `--results` JSON inputs. See `references/phase2/steps/release-gate.md` for full stage instructions.
